...
export default createRoute(async (c) => {
  ...
  try {
    // 1. CARI DATA HALAMAN HOME (Untuk Title/Meta jika ada)
    const page: any = await db.prepare("SELECT * FROM pages WHERE slug = '/' OR slug = 'home'").first();
    const pageTitle = page ? page.title : 'Home';

    // 2. AMBIL WIDGET MILIK HOMEPAGE
    const { results: widgets } = await db.prepare(
      "SELECT * FROM frontpage_widgets WHERE page_id = 'home' AND is_active = 1 ORDER BY display_order ASC"
    ).all();
    ...
    return c.html(
      <html lang="id">
        <head>
          ...
          <title>{pageTitle} | Visoe Luxury</title>
        ...
        <body>
          <div class="pb-24">
            ...
            {await Promise.all(widgets.map(async (widget) => {
              ...
              // --- 2. HERO SLIDER (BOXED / TIDAK FULL WIDTH) ---
              if (widget.widget_type === 'hero_slider') {
                return (
                  <div class="max-w-[1400px] mx-auto px-4 md:px-6 mt-6 md:mt-10">
                    <div class="swiper hero-slider h-[450px] md:h-[750px] overflow-hidden relative shadow-sm">
              ...
              if (widget.widget_type === 'icon_nav') {
                return (
                  <div class="max-w-[1400px] mx-auto px-4 md:px-6 text-center mt-12 md:mt-20">
              ...
              if (widget.widget_type === 'new_arrivals' || widget.widget_type === 'featured_products') {
                ...
                return (
                  <div class="max-w-[1400px] mx-auto px-4 md:px-6 mt-24">
                      ...
                      {products.map((p: any) => {
                        ...
                        return (
                          <a href={`/products/${p.slug}`} class="group">
                            <div class="aspect-[3/4] overflow-hidden bg-neutral-50 mb-6 relative shadow-sm border border-neutral-100">
              ...
              if (widget.widget_type === 'custom_title' || widget.widget_type === 'custom_paragraph') {
                ...
                return (<div class="max-w-[1400px] mx-auto px-4 md:px-6" style={{ textAlign: data.align || 'center' }}><Tag class={styleClass} style={{ color: data.text_color }}>{data.text}</Tag></div>);
              }

              // --- 9. TRUST BADGES ---
              if (widget.widget_type === 'trust_badges') {
                return (
                  <div class="max-w-[1400px] mx-auto px-4 md:px-6 grid grid-cols-1 md:grid-cols-3 gap-12 text-center border-t border-neutral-100 pt-20 mt-20">
          ...
          {/* SCRIPT JS UNTUK DRAWER, SLIDER, DLL */}
