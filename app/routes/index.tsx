...
export default createRoute(async (c) => {
  ...
  try {
    // 1. FIX: AMBIL NAMA WEBSITE DARI TABEL store_settings (Bukan settings)
    ...
    // 2. CARI DATA HALAMAN HOME
    const page: any = await db.prepare("SELECT * FROM pages WHERE slug = '/' OR slug = 'home'").first();
    const pageTitle = page ? page.title : 'Home';

    // 3. AMBIL WIDGET HEADER & FOOTER (GLOBAL)
    ...
    if (!headerData.menu || headerData.menu.length === 0) {
      headerData.menu = [{ title: 'Shop', url: '/products', columns: [] }];
    }
    if (!footerData.columns || footerData.columns.length === 0) {
      footerData.columns = [{ title: 'Company', links: [{ name: 'About Us', url: '/about' }] }];
    }

    // 4. AMBIL WIDGET KONTEN DI TENGAH
    const { results: widgets } = await db.prepare(
      "SELECT * FROM frontpage_widgets WHERE page_id = 'home' AND widget_type NOT IN ('header_widget', 'footer_widget') AND is_active = 1 ORDER BY display_order ASC"
    ).all();
        ...
        <body class="flex flex-col min-h-screen">
          ...
          <main class="flex-1 pb-24">
            {await Promise.all(widgets.map(async (widget) => {
              ...
              if (widget.widget_type === 'hero_slider') {
                return (
                  <div class="max-w-[1200px] mx-auto px-4 md:px-6 mt-6 md:mt-10">
                    <div class="swiper hero-slider h-[250px] md:h-[450px] overflow-hidden relative shadow-sm">
                      <div class="swiper-wrapper">{data.slides?.map((s: any) => (<div class="swiper-slide relative"><img src={s.image} class="w-full h-full object-cover" loading="lazy" /><div class="absolute inset-0 bg-black/5"></div><a href={s.link || '#'} class="absolute inset-0"></a></div>))}</div>
                    ...
                    <div class="flex flex-wrap justify-center gap-6 md:gap-12">
                      {data.items?.map((item: any) => (
                        <a href={item.link || '#'} class="flex flex-col items-center group w-20 md:w-28">
                          <div class="w-16 h-16 md:w-24 md:h-24 rounded-full border border-neutral-100 p-1 group-hover:border-black transition-all bg-white overflow-hidden shadow-sm"><img src={item.image} class="w-full h-full object-cover rounded-full" alt={item.title} /></div>
              ...
              if (widget.widget_type === 'new_arrivals' || widget.widget_type === 'featured_products') {
                ...
                return (
                  <div class="max-w-[1200px] mx-auto px-4 md:px-6 mt-24">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                      {data.promos?.map((promo: any) => (
                        <div class="group space-y-6 text-center">
                          <a href={promo.link || '#'} class="block aspect-square md:aspect-[4/5] overflow-hidden border border-neutral-100 shadow-sm relative"><img src={promo.image} class="w-full h-full object-cover group-hover:scale-105 transition-all duration-1000" alt="Promo" /></a>
                ...
                return (
                  <div class="max-w-[1200px] mx-auto px-4 md:px-6 grid grid-cols-1 md:grid-cols-3 gap-12 text-center border-t border-neutral-100 pt-20 mt-20">
                    {data.badges?.map((badge: any) => (
                      <div class="space-y-4">
                        {badge.image && <img src={badge.image} class="w-12 h-12 mx-auto object-contain mb-4" alt="Trust Badge" />}
          ...
          <script dangerouslySetInnerHTML={{ __html: `
            ...
            document.addEventListener('DOMContentLoaded', () => {
              if(document.querySelector('.hero-slider')) new Swiper('.hero-slider', { loop: true, autoplay: { delay: 6000 }, pagination: { el: '.swiper-pagination', clickable: true }, effect: 'fade' });
              window.updateCartUI();
